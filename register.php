...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($username_account) && !empty($password) && !empty($hospital_code) && !empty($hospital_name)) {
        try {
            // ตรวจสอบว่ามีข้อมูลซ้ำกันในฐานข้อมูลหรือไม่ (username_account, hospital_code, และ hospital_name)
            ...
            if ($stmt->rowCount() > 0) {
                echo '<p class="text-red-500 text-center">ชื่อผู้ใช้, รหัสสถานพยาบาล หรือชื่อ รพ.สต นี้มีอยู่ในระบบแล้ว กรุณาเลือกข้อมูลอื่น</p>';
            } else {
                // บันทึกข้อมูลลงตาราง users
                $stmt = $con->prepare("INSERT INTO users (username_account, password, hospital_code, hospital_name) VALUES (:username_account, :password, :hospital_code, :hospital_name)");
                $stmt->bindParam(':username_account', $username_account);
                ...
                echo '<p class="text-green-500 text-center">ลงทะเบียนสำเร็จ!</p>';
        ...
        echo '<p class="text-red-500 text-center">กรุณากรอกข้อมูลให้ครบถ้วน</p>';
    }
}
?>
