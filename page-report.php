...
// ตรวจสอบว่ามีการตั้งค่า `hospital_name` ในเซสชันหรือไม่
if (!isset($_SESSION['hospital_name'])) {
    // ดึง `hospital_name` ของผู้ใช้จากฐานข้อมูล
...
// กำหนดค่า hospital_name
$hospital_name = $_SESSION['hospital_name'];

// กำหนดค่าเริ่มต้นให้ตัวแปร $withdrawRecords
$withdrawRecords = [];

try {
    // ดึงข้อมูลใบเบิกที่ `dept_id` ตรงกับ `hospital_name` ของผู้ใช้ที่ล็อกอินเข้ามา
        $stmt = $con->prepare("SELECT po_number, MAX(date) as date 
        FROM po 
